...
public class MarkdownsTestCase {
    ...
    public void space() {
        if (!Markdowns.MARKED_AVAILABLE) {
            final String md = "Sym是一个用Java写的实时论坛，欢迎来[体验Sym](${servePath})！";
            final String html = Markdowns.toHTML(md);

            Assert.assertEquals(html, "<p>Sym 是一个用 Java 写的实时论坛，欢迎来<a href=\"${servePath}\">体验 Sym</a>！</p>");
        }
    }
    ...
    public void toHTML1() {
        if (!Markdowns.MARKED_AVAILABLE) {
            ...
            md = "[link](${servePath}/blob/master/README_zh_CN.md)";
            html = Markdowns.toHTML(md);
            Assert.assertEquals(html, "<p><a href=\"${servePath}/blob/master/README_zh_CN.md\">link</a></p>");
    ...
    public void timeout() throws Exception {
        if (!Markdowns.MARKED_AVAILABLE) {
            ...
            Assert.assertEquals(html, "Content render failed, please <a href=\"${servePath}/article/1438049659432\">report</a> this problem to help us enhance it, thank you &hearts;");
        }
    }
}
